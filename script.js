...
function mostrarRankingGlobal() {
    ...
    const maxPalabras = 10;
    const palabrasAMostrar = rankingOrdenado.slice(0, maxPalabras);

    if (palabrasAMostrar.length === 0) {
        resultado.innerHTML += "<p>No se han introducido planes todavía.</p>";
    } else {
        palabrasAMostrar.forEach(([palabra, count], index) => {
    ...
    // Renderizar gráfico con los datos (opcional)
...
// Aplicar tema guardado en el almacenamiento local al cargar la página
